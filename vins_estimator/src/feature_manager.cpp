...
/**
 * @description: 获得窗口中被跟踪的特征数量，统计的是特征的数目
 * @param {type} 
 * @return: 
 */
...
bool FeatureManager::addFeatureCheckParallax(int frame_count, const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, double td)
{
    ...
    if (frame_count < 2 || last_track_num < 20)
        return true;
    //计算每个特征在次新帧和次次新帧中的视差，last frame and third last frame
...
/**
 * @description: 得到frame_count_l与frame_count_r两帧之间的对应特征点，用于标定相机外参
 * @param {type} 
 * @return: 
 */
...
/**
 * @description: 剔除feature中估计失败的点（solve_flag == 2）
 * @param {type} 
 * @return: 
 */
...
/**
 * @description: 对特征点进行三角化求深度（SVD分解）
 * @param {type} 
 * @return: 
 */
void FeatureManager::triangulate(Vector3d Ps[], Vector3d tic[], Matrix3d ric[])
{
    for (auto &it_per_id : feature)
    {
        ...
        // ROS_ASSERT断言所提供的表达式的计算结果为true。 如果为假，程序将中止执行，并在哪个文件中给出有关哪个断言失败的信息性语句。
...
/**
 * @description: 边缘化最老帧时，直接将特征点所保存的帧号向前滑动
 * @param {type} 
 * @return: 
 */
void FeatureManager::removeBack()
{
         ...
         it != feature.end(); it = it_next)
    {
        ...
        //如果feature_per_frame为空则直接删除特征点,因为特征点已经没有对应帧了
        else
        {
            it->feature_per_frame.erase(it->feature_per_frame.begin());
